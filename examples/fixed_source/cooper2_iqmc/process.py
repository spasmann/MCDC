...
for i in range(Nx):
    ...
    for j in range(Ny):
        ...
        z4[i,j] = q[zonex, zoney] + qdotx[zonex, zoney]*(x[i] - midx) + qdoty[zonex, zoney]*(y[j] - midy)  
...
X, Y = np.meshgrid(x_mid, x_mid)
interp = RegularGridInterpolator((x_mid, x_mid), q,method="linear",bounds_error=False,fill_value=None)
